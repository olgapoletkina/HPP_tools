...
Description:
This script identifies Revit link elements (RVT files) 
in the project and removes them if the document is not 
workshared. It then outputs a message listing the 
names of the deleted RVT files, if any were found.
___________________________________________________________
How-to:
Press the button.
___________________________________________________________
...
for link in links:
    if '.rvt' in link.Parameter[DB.BuiltInParameter.ALL_MODEL_TYPE_NAME].AsString():
...
if doc.IsWorkshared == True:
    ...
    print('The following RVT files were removed:')
    for link in links_to_remove_names:
        print(link)
    with DB.Transaction(doc, 'Delete all RVT') as t:
        t.Start()
    ...
    print('No attached RVT were detected!')